...
description: >-
  Runs a human UI review on the live app with livepin, applies simple fixes
  immediately, and records complex fixes for later. Use when the user asks for a human review of the UI, to
...
- Show the running UI to the human with livepin, apply simple fixes immediately, and record complex fixes for later.
...
- Explain that simple fixes happen immediately without a feedback record; complex fixes are recorded for later.
...
- Apply simple, clear, localized fixes that can be verified during the review, such as a copy correction, spacing adjustment, or isolated style fix.
...
- Do not add successfully applied simple fixes to feedback documents or create plan tasks for them.
- Record complex fixes for later when they require broader changes, investigation, or product/design decisions; acknowledge the issue in its thread without implementing it during the review.
- If a fix proves complex or cannot be completed and verified during the review, defer it and include its remaining work in the record.
...
- Write only deferred issues to the feedback record; exclude successfully applied simple fixes from entries, counts, and themes.
