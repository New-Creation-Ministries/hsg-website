...
- Document title is `Events | Holy Spirit Generation`. `h1` is Events. The IST note is “Times are in IST (Asia/Kolkata). Dated gatherings sit on the line. Sunday services repeat and stay off it.”
...
- Sunday `h2` is “Sunday services”. Intro is “Not on the dated line. An add applies every Sunday in IST. Subscribing follows a later published time after your calendar refreshes.” Each Home “New to HSG?” item keeps name, language, and time, including “8–9am” and “9:30am onwards”. The page text does not contain `1:30`.
...
status: pending
acceptance_criteria:
- `e2e/events.spec.ts` opens `/events` from Home “What’s going on” and from the header Events link. Events is `aria-current="page"`. Back returns to Home. The shell sentence and “Placeholder photograph” are absent. “No upcoming events.” is visible while `events` is empty. Both Sunday services show Home time text, one Add to calendar each, and no “1:30”.
