...
Registers this standalone module into a project. Module identity (name, code, version) comes from `assets/module.yaml`. Collects user preferences and writes them to three files:
...
Lavish is a hard dependency, not an optional review provider. Verify that the installed agent skills include `lavish`, Node.js is version 22 or newer, and `lavish-axi` is version `0.1.45`. When the CLI is absent or at another version, install the pinned runtime with `npm install -g lavish-axi@0.1.45`; if installation requires host approval, request it. Run `env LAVISH_AXI_STATE_DIR="{project-root}/_bmad-output/.lavish-axi" lavish-axi design` as the preflight. If any check fails after one repair attempt, stop setup and report the dependency failure; do not register a screenshot-only or browser-only fallback.
...
Ask using the prompt with its default value. Apply declared `result` templates when storing. Fields with `user_setting: true` go exclusively to `config.user.yaml`.
...
Use the script JSON output to display what was written — config values set (written to `config.yaml` at root for core, module section for module values), user settings written to `config.user.yaml` (`user_keys` in result), help entries added, fresh install vs update, and the verified Lavish skill and CLI version.
